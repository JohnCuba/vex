...
export const defineServerEntryPoint = <THandlerModule>(
  renderer: ServerAppRenderer<THandlerModule>,
) => renderer;

declare const __VEX_ROUTE__: string;

type ClientAppRenderer<THandlerModule> = (module: THandlerModule) => Promise<void>;
export const defineClientEntryPoint = async <THandlerModule>(
  renderer: ClientAppRenderer<THandlerModule>,
) => {
